...
const Admin3 = () => {
    ...
    //       const token = localStorage.getItem("token")
    //       axios.delete(`http://localhost:5000/onedelete?project_name_id=${id}`,{
    //         headers:{
    //           Authorization:`Bearer ${token}`,
    //         },
    //       })
    ...
    const Formsummit = () => {
        ...
        setPay(Owasp)
        axios
            .post(`http://localhost:5000/payload`, { Owasp },{
                headers: {
                    Authorization:`Bearer ${token}`,
                    'Access-Control-Allow-Origin' : '*',
                    'Content-Type': 'application/json'
...
const Formsummit2 = () => {
    console.log(`2${Owasp}`);
        setPay(Owasp)
        axios
            .post(`http://localhost:5000/payload2`, { payloadone,Owasp,valuepayload },{
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Access-Control-Allow-Origin' : '*',
                    'Content-Type': 'application/json'
    ...
    const FormsummitAll2 = () => {
        ...
        setPay(Owasp)
        axios
            .post(`http://localhost:5000/payload3`, { Owasp,payloadall },{
                headers: {
                    Authorization:`Bearer ${token}`,
                    'Access-Control-Allow-Origin' : '*',
                    'Content-Type': 'application/json'
    ...
    const ShowFormsummit = () => {
        ...
        setPay(Owasp)
        axios
            .post(`http://localhost:5000/payload4`, {Owasp},{
                headers: {
                    Authorization:`Bearer ${token}`,
                    'Access-Control-Allow-Origin' : '*',
                    'Content-Type': 'application/json'
    ...
    //     setPay(Owasp)
    //     axios
    //         .post(`http://localhost:5000/payload3`, { Owasp })
    //         .then(response => {
    //         alert("บันทึกสำเร็จ")
...
{pay&& (
    ...
    <TextArea  cols={200} placeholder='{"KEY":["Value","Value"","Value"","Value"","Value"","Value"","Value"","Value"]}' type="text" className="from-control" e={payloadall} onChange={(e)=>setPayloadall(e.target.value)}/>
    </Form.Item>
    <Form.Item>                
    <button  className='btn'type="submit" value="บันทึก">OK</button>
    </Form.Item>
