...
async def post_response(url,cookies_data,postbody=None):
    try:
        response = requests.post(url, headers={'User-Agent'
...
async def save_log(response, i,project_name,user,Host,http_log_data,formlist=None, state='c'):
    try:   
        ...
        insert_query = (
        "INSERT INTO urllist "
        "(URL, method, URI, Host, HTTPVer, status_code, reason,state,length, isredirect,redirect_to, ActionFound, "
        "req_header, req_body, res_header, res_body, PID) "
                                ...
                                try:
                                    ...
                                    if response.status_code == 500:         
                                       ...
                                       password='',
                                        ...
                                        insert_query = (
                                        "INSERT INTO att_ps "
                                        "(URL_ID, PID, OID, URL, position, state, payload, status_code, reason, "
                                        "res_header, res_body, req_header, req_body, method, URI, length, vul_des, "
                                        "vul_sol, vul_ref, OType, Vul_name, Severity) "
                ...
                for line in f:
                    ...
                    for Server_data in pathtraversal:
                        ...
                        if Server_word.lower() in Server_data[1].lower():                        
                            ...
                            values = (Server_data[3], Server_data[2], '4',
                                       Server_data[0], 'T', Server_data[1],sql2_[0][0],
                                       sql2_[0][1],sql2_[0][2],sql2_[0][3],sql2_[0][4],sql2_[0][5])
                            mycursor.execute(insert_query, values)
                            mydb.commit()
                        else:
    ...
    try:
        ...
        if os.path.exists(file_path):
            try:
                ...
                lines = [line for line in lines if '(Status: 500)' not in line and '(Status: 429)' not in line and '(Status: 403)'
                        not in line and '(Status: 400)' not in line and '(Status: 301)' not in line and '()']
        ...
        if os.path.exists(file_path):
            try: 
                with open(file_path, "r") as f:
                    lines = f.readlines()
                lines = [line.replace('', ' ') for line in lines]
                lines = [line for line in lines if '(Status: 500)' not in line and '(Status: 429)' 
                         not in line and '(Status: 403)' not in line and '(Status: 400)' 
                         not in line and '(Status: 301)' not in line and '()']
                with open(file_path, 'w') as file:
                    file.writelines(lines)
                with open(file_path, "r") as f:
                    j = 1
                    for x in f:
                        ...
                        mycursor = mydb.cursor()
                        insert_query = (=
                        "INSERT INTO att_ps (URL_ID, position , PID, OID, "
                        "URL, state, payload, vul_des, vul_sol, vul_ref, "
                        "OType, Vul_name, Severity) "
                        "VALUES (%s, %s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
                         )
                        values = (url_ID[0], url ,project_name_id, '7', scope_url, 
                                  'T', x.split()[0], sql2_[0][0], sql2_[0][1], sql2_[0][2], sql2_[0][3], sql2_[0][4], sql2_[0][5])
                        mycursor.execute(insert_query, values)
                        mydb.commit()
                if os.path.exists(file_path):
                            try: 
    ...
    try:
        ...
        for Server_data in Server:
            ...
            if Server_data[1].find(Server_word) != -1:
                ...
                mycursor = mydb.cursor()
                insert_query = (
                "INSERT INTO att_ps (URL_ID, PID, OID, URL, state, res_header, "
                "vul_des, vul_sol, vul_ref, OType, Vul_name, Severity) "
                "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
                )
                values = (Server_data[3], Server_data[2], '1', Server_data[0], 'T', 
                          Server_data[1], sql2_[0][0], sql2_[0][1], sql2_[0][2], sql2_[0][3], sql2_[0][4], sql2_[0][5])
                mycursor.execute(insert_query, values)
                mydb.commit()
            else:
                print("ไม่พบ Server")
    ...
    try:
        ...
        for HSTS_data in PTarget:
            ...
            if HSTS_word[0].lower() in HSTS_data[1].lower():
                ...
                values = (HSTS_data[3],HSTS_data[2],'8',HSTS_data[0],'T',
                          HSTS_data[1],sql2_[0][0],sql2_[0][1],sql2_[0][2],sql2_[0][3],sql2_[0][4],sql2_[0][5])
                mycursor.execute(insert_query, values)
                mydb.commit()
            ...
            for payload in file:
                ...
                for HSTS_data in PTarget:
                    ...
                    if detected_payload.lower() in str(HSTS_data[1]).lower():
                        ...
                        insert_query = (
                            "INSERT INTO att_ps (URL_ID, PID, OID, URL, state, res_header, "
                            "vul_des, vul_sol, vul_ref, OType, Vul_name, Severity, payload) "
                            "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
                        )
                        values = (HSTS_data[3],HSTS_data[2],'9',HSTS_data[0],'T',
                                  HSTS_data[1],sql2_[0][0],sql2_[0][1],sql2_[0][2],sql2_[0][3],sql2_[0][4],sql2_[0][5],detected_payload)
                        mycursor.execute(insert_query, values)
                        mydb.commit()
                        return 
                    else:
                ...
                for HSTS_data in PTarget:
                    ...
                    try:    
                        if detected_payload.lower()+"." in encoded_data.lower()+"." :
                            ...
                            mycursor = mydb.cursor()
                            insert_query = (
                            "INSERT INTO att_ps (URL_ID, PID, OID, URL, state, "
                            "vul_des, vul_sol, vul_ref, OType, Vul_name, Severity, payload) "
                            "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
                                )
                            values = (HSTS_data[3],HSTS_data[2],'9',
                                      HSTS_data[4],'T',sql2_[0][0],sql2_[0][1],
                                      sql2_[0][2],sql2_[0][3],sql2_[0][4],sql2_[0][5],detected_payload)
                            mycursor.execute(insert_query, values)
                            mydb.commit()
                            break  
                    except Exception as decode_error:
        ...
        for Server_data in Server:
            ...
            if Server_data[1].find(Set_Cookie) != -1:
                ...
                if  Server_data[1].find(Secure_Header) != -1:
                    ...
                    values = (Server_data[3],Server_data[2],'2',
                              Server_data[0],'T',Server_data[1],sql2_[0][0],sql2_[0][1],sql2_[0][2],sql2_[0][3],sql2_[0][4],sql2_[0][5])
                    mycursor.execute(insert_query, values)
                    mydb.commit()
                if Server_data[1].find(HttpOnly_Header) != -1:
                    ...
                    values = (Server_data[3],Server_data[2],'3',
                              Server_data[0],'T',Server_data[1],sql3_[0][0],sql3_[0][1],sql3_[0][2],sql3_[0][3],sql3_[0][4],sql3_[0][5])
                    mycursor.execute(insert_query, values)
                    mydb.commit()
                ...
                if Server_data[1].find(Expires_Header) != -1:
                    ...
                    values = (Server_data[3],Server_data[2],'5',
                              Server_data[0],'T',Server_data[1],sql5_[0][0],sql5_[0][1],sql5_[0][2],sql5_[0][3],sql5_[0][4],sql5_[0][5])
                    mycursor.execute(insert_query, values)
                    mydb.commit()
                ...
                if Server_data[1].find(SameSite_Header) != -1:
                    ...
                    values = (Server_data[3],Server_data[2],'6',Server_data[0],'T',Server_data[1],sql6_[0][0],
                              sql6_[0][1],sql6_[0][2],sql6_[0][3],sql6_[0][4],sql6_[0][5])
                    mycursor.execute(insert_query, values)
                    mydb.commit()
            ...
            for url_data in URL_data:
                ...
                if url_data[1] == 'POST':
                    ...
                    if url_data[2] != None :
                        ...
                        # print(att_url)
                        try:      
                            await brutecommand(att_url,baseper,att_params,att_paramsname,select_url_id_data,baseatt_URL,project_name, user
                                               ,cookies_data,url_data[2],uri)
                        except Exception as e:
                            print(f"brutepathtraversal: {e}")
                        # if vres and vparams != None:
                        #     print(vres)
...
async def onedata():
    try:
        ...
        select_att_ID_sql = "SELECT URL , payload ,position ,Vul_des ,"
        " Vul_sol , OType , ATT_ID,vul_ref,Severity,vul_name,state FROM att_ps WHERE PID = %s AND OID = %s "
        mycursor.execute(select_att_ID_sql, (project_name_id, '11'))
        select_att_ID_sql_DATA = mycursor.fetchall()       
        if select_att_ID_sql_DATA:
            mycursor = mydb.cursor()
        ...
        mycursor = mydb.cursor()
        userall ="SELECT username FROM user WHERE role = 'Advance'"
...
def payload3():
    try:
        ...
        admin = user.get('role', None)
        if (admin == 'Admin'):
            payload_ = request.json['selectedFile']
            Owasp = request.json['Owasp']
            inputKey = request.json['inputKey']
            print(Owasp,payload_)
            data = {f"{inputKey}": [line.strip() for line in payload_.split("\n")]}
